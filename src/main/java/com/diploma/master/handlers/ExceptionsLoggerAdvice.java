...
public class ExceptionsLoggerAdvice {
  ...
  public void logExceptionAdvice(ExceptionHandler exceptionHandler, Exception ex) {
    ...
    logExceptionAdvice(ex, request);
  }

  private void logExceptionAdvice(Exception ex, HttpServletRequest request) {
    if (request != null) {
  ...
  private String requestToString(HttpServletRequest request) {
    StringBuilder sb =
        new StringBuilder("HttpRequest{" + request.getMethod() + " " + request.getRequestURI() +
            "\nrequestHeaders: \n");
