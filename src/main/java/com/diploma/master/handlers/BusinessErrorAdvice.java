package com.diploma.master.handlers;

import com.diploma.master.models.exceptions.ErrorResponse;
...
public class BusinessErrorAdvice {
  ...
  public List<ErrorResponse> handleMethodArgumentNotValidException(
    ...
    return createErrorResponseFromMethodArgumentNotValidException(ex,
  ...
  public List<ErrorResponse> handleResourceNotFoundException(ResourceNotFoundException ex) {
    ...
    return Collections.singletonList(
        new ErrorResponse(ex.getMessage(), HttpStatus.NOT_FOUND.value(), stackTrace));
  }
  ...
  private List<ErrorResponse> createErrorResponseFromMethodArgumentNotValidException(
      MethodArgumentNotValidException ex, int status) {
    String stackTrace = isStackTraceEnabled ? ExceptionUtils.getStackTrace(ex) : null;
